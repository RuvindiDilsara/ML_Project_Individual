...
Original file is located at
    https://colab.research.google.com/drive/1YlgmlSZ7k1pGOoCC-Rz06C73KMkmIw54
"""
...
WORKING_DIR=f"{MOUNT_PATH}/MyDrive/ML/ML_Project_Individual/Layer_9"
DATASET_DIR=f"{MOUNT_PATH}/MyDrive/ML/ML_Group_Project"

train = pd.read_csv(f"{DATASET_DIR}/layer_9_train.csv")
train.head()

valid = pd.read_csv(f"{DATASET_DIR}/layer_9_valid.csv")
valid.head()

test = pd.read_csv(f"{DATASET_DIR}/layer_9_test.csv")
test.head()
...
# Hyperparameter tuning for a classifier (e.g., Random Forest)
param_grid = {
...
grid_search.fit(x_train_trf, y_train[L1])

# Best hyperparameters
best_params = grid_search.best_params_

"""Train a Random Forest Classifier after hyperparameter tuning"""

# Step 5: Build and train the classifier with the best hyperparameters
best_rf_classifier = RandomForestClassifier(random_state=42, **best_params)
best_rf_classifier.fit(x_train_trf, y_train)

# Evaluate the classifier
y_pred = best_rf_classifier.predict(x_valid_trf)
print(classification_report(y_valid[L1], y_pred))
...
"""Testing draft code piece"""

# Step 7: Dimensionality reduction (e.g., PCA)
pca = PCA(n_components=10)
X_train_pca = pca.fit_transform(x_train_trf)
X_val_pca = pca.transform(x_valid_trf)

# Step 8: Cross-validation
cross_val_scores = cross_val_score(best_rf_classifier, X_train_pca, y_train[L1], cv=5)
...
joblib.dump(best_clf_L1, f'{WORKING_DIR}/best_model_L1.joblib')
...
label_1_test_trf = pd.DataFrame(pca.transform(x_test[L1]))
# Assuming label_2_x_test_trf is your test data that you want to make predictions on
label_1_y_test_pred = best_model.predict(label_1_test_trf)

# Create a DataFrame with the test data and predictions
combined_df = pd.DataFrame({
    'ID': test_df['ID'],
    'label_1': label_1_y_test_pred})

# combined_df = pd.concat([test_df, pd.DataFrame({'Predicted_Label': label_1_y_test_pred})], axis=0)
# Save the DataFrame to a CSV file
# combined_df.to_csv('layer_9_label_2.csv', index=False)

# Save the DataFrame to the specified CSV file path
combined_df.to_csv(f"{WORKING_DIR}/layer_9_label_1.csv", index=False)
...
label_2_random_search = RandomizedSearchCV(
    estimator=label_2_svm, param_distributions=param_dist, scoring='accuracy', cv=5, verbose=1, n_jobs=-1, n_iter=2
)
...
best_clf_L2 = SVC(C=63.0957344480193, gamma=0.25118864315095796, kernel='poly')
...
combined_df = pd.DataFrame({
    'ID': test_df['ID'],
    'Predicted_Label': label_2_y_test_pred})
...
# Save the DataFrame to the specified CSV file path
combined_df.to_csv(f"{WORKING_DIR}/layer_9_label_2.csv", index=False)
...
combined_df = pd.DataFrame({
    'ID': test_df['ID'],
    'Predicted_Label': label_3_y_test_pred})
...
# Save the DataFrame to the specified CSV file path
combined_df.to_csv(f"{WORKING_DIR}/layer_9_label_3.csv", index=False)
...
label_4_random_search.fit(label_3_x_train_trf, y_train[L4])
...
combined_df = pd.DataFrame({
    'ID': test_df['ID'],
    'Predicted_Label': label_4_y_test_pred})
...
# Save the DataFrame to the specified CSV file path
combined_df.to_csv(f"{WORKING_DIR}/layer_9_label_4.csv", index=False)
...
# Save the DataFrame to the specified CSV file path
output_df.to_csv(f"{WORKING_DIR}/190140L_layer_9.csv", index=False)
...
# Save the DataFrame to the specified CSV file path
predict_df.to_csv(f"{WORKING_DIR}/prediction_layer_9.csv", index=False)
...
# Save the DataFrame to the specified CSV file path
predict_df_L2.to_csv(f"{WORKING_DIR}/prediction_layer_9_L2.csv", index=False)